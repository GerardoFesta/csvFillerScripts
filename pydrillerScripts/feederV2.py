...
class Feeder:
    ...
    def feed(self):
        ...
        for idx in self.df.index:
            ...
            d1=datetime.strptime(self.finaldf.at[idx, "Data inizio"], "%Y-%m-%d %H:%M:%S %z")
            d2=datetime.strptime(self.finaldf.at[idx, "Data fine"], "%Y-%m-%d %H:%M:%S %z")
            if(d1==d2):
                risultati_versione=MinerSingolo.mineVersion(self.repo, self.df.at[idx,"Primo commit"], self.df.at[idx,"Ultimo commit"])
            else:
                risultati_versione=Miner.mineVersion(self.repo, d1,d2)
